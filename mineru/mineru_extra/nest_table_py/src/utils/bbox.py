...
def overlap_relation(rect1, rect2):
        ...
        if rect1_contains_rect2:
            # 2 是 1 的子集
            return 1 
        if rect2_contains_rect1:
            # 1 是 2 的子集
            return 2
        else:
            # 两个矩形不相交
            return 0
# 找到字典中最小的bbox
def find_min_bbox_area(data_dict):
        min_area = float('inf')
        min_key = None
        min_bbox = None

        for key, bbox in data_dict.items():
